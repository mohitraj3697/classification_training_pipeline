...
def load_hf_weights(custom_gpt):
    ...
    for i, block in enumerate(custom_gpt.trf_blocks):
        ...
        qkv_w = sd[p + "attn.c_attn.weight"]  # (768, 2304)
        qkv_b = sd[p + "attn.c_attn.bias"]    # (2304,)
...
for param in model.parameters():                              #frezze model parameters
...
num_classes = 2              #no. of classification 
model.out_head = torch.nn.Linear(in_features=BASE_CONFIG["emb_dim"], out_features=num_classes)     #unfreeze output layer


for param in model.trf_blocks[-1].parameters():         #unfreeze last transformer block
    param.requires_grad = True

for param in model.final_norm.parameters():     #unfreeze final layer norm
    param.requires_grad = True
...
print( outputs[:, -1, :])   #last token logits
